...
def extract_clap_scores(args) -> dict[str, float]:
    clap_scores_json_fpath = os.path.join(
        args.samples_dir, "rewards/clap_scores.json"
    )
    ...
    if os.path.exists(clap_scores_json_fpath):
        print(
            f"CLAP scores already extracted and saved to "
            f"{clap_scores_json_fpath}"
        )
    ...
    for json_file in tqdm.tqdm(
        ...
        with open(json_fpath) as f:
            ...
            assert os.path.exists(audio_fpath), (
            ...
            sample_id_2_clap_scores.update(
                {
                    ...
                    for sample_id, score in zip(
                        sample_ids, clap_scores, strict=False
                    )
                }
            )
            texts = []
    ...
    print(
        f"finished processing {total_processed} samples for CLAP scores"
    )
    with open(clap_scores_json_fpath, "w") as f:
        print(
            f"saving {len(sample_id_2_clap_scores)} CLAP scores to "
            f"{clap_scores_json_fpath}"
        )
...
def extract_audiobox_aesthetics_scores(args) -> dict[str, float]:
    ...
    os.makedirs(
        os.path.dirname(audiobox_aesthetics_json_fpath), exist_ok=True
    )
    print(
        f"audiobox_aesthetics_json_fpath: "
        f"{audiobox_aesthetics_json_fpath}"
    )
    if os.path.exists(audiobox_aesthetics_json_fpath):
        print(
            f"Audiobox aesthetics scores already extracted and saved to "
            f"{audiobox_aesthetics_json_fpath}"
        )
    ...
    for json_file in tqdm.tqdm(
        ...
        if len(sample_ids) == args.batch_size:
            ab_input_list = [
                {"path": audio, "sample_rate": sr}
                for audio, sr in zip(audios, srs, strict=False)
            ]
            ...
            sample_id_2_ab_scores.update(
                {
                    ...
                    for sample_id, ab_scores_dict in zip(
                        sample_ids, ab_scores, strict=False
                    )
                }
            )
            audios = []
    ...
    print(
        f"finished processing {total_processed} samples for Audiobox "
        f"aesthetics scores"
    )
    with open(audiobox_aesthetics_json_fpath, "w") as f:
        print(
            f"saving {len(sample_id_2_ab_scores)} Audiobox aesthetics "
            f"scores to {audiobox_aesthetics_json_fpath}"
        )
...
def extract_semantic_consistency_scores(args) -> dict[str, float]:
    ...
    os.makedirs(
        os.path.dirname(semantic_consistency_json_fpath), exist_ok=True
    )
    print(
        f"semantic_consistency_json_fpath: "
        f"{semantic_consistency_json_fpath}"
    )
    if os.path.exists(semantic_consistency_json_fpath):
        print(
            f"Semantic consistency scores already extracted and saved to "
            f"{semantic_consistency_json_fpath}"
        )
            ...
            sample_id_2_semantic_consistency_scores.update(
                {
                    ...
                    for sample_id, scores_dict_i in zip(
                        sample_ids, scores_dicts, strict=False
                    )
                }
            )
            audios = []
    ...
    print(
        f"finished processing {total_processed} samples for Semantic "
...
def extract_rewards(args) -> dict[str, float]:
    ...
    assert "audiobox_aesthetics" in args.rewards, (
        "Audiobox aesthetics is required for MRSD"
    )
    assert "semantic_consistency" in args.rewards, (
    ...
    sample_id_2_semantic_consistency_scores = (
        extract_semantic_consistency_scores(args)
    )
    ...
    for sample_id in tqdm.tqdm(common_sample_ids):
        sample_id_2_rewards[sample_id] = {
            ...
            "audiobox_aesthetics": (
                sample_id_2_ab_scores[sample_id]
            ),
            "semantic_consistency": (
                sample_id_2_semantic_consistency_scores[sample_id]
            ),
        }
...
def bin_by_prompt(
    ...
    if os.path.exists(samples_by_prompt_fpath):
        print(
            f"Samples by prompt already extracted and saved to "
            f"{samples_by_prompt_fpath}"
        )
...
def extract_rewards2thresholds(
    ...
    os.makedirs(
        os.path.dirname(rewards2abs_diffs_fpath), exist_ok=True
    )
    print(f"rewards2abs_diffs_fpath: {rewards2abs_diffs_fpath}")
    if os.path.exists(rewards2abs_diffs_fpath):
        print(
            f"Rewards2abs diffs already extracted and saved to "
            f"{rewards2abs_diffs_fpath}"
        )
...
def balance_primary_axes(
    ...
    min_mrsd_pairs = min(
        len(mrsd_pairs)
        for mrsd_pairs in primary_reward_2_mrsd_pairs.values()
    )
    ...
    for primary_reward in args.rewards:
        ...
        mrsd_pairs_balanced = random.sample(
            mrsd_pairs, min_mrsd_pairs
        )
        primary_reward_2_mrsd_pairs_balanced[primary_reward] = (
            mrsd_pairs_balanced
        )
...
def write_dataset_json_lines(
    ...
    for split_name, split_pairs in splits.items():
        ...
        print(
            f"Writing {len(split_pairs)} pairs to {output_fpath}"
        )

        with open(output_fpath, "w") as f:
            for primary_reward, prompt, pos_sample, neg_sample in tqdm.tqdm(
                split_pairs, desc=f"Writing {split_name}"
            ):
                entry = create_jsonl_entry(
                    primary_reward, prompt, pos_sample, neg_sample
                )
...
def construct_mrsd_dataset(args):
    ...
    rewards2thresholds = extract_rewards2thresholds(
        args, samples_by_prompt
    )
    primary_reward_2_mrsd_pairs: dict[str, list[tuple[str, dict, dict]]] = (
        find_mrsd_pairs(args, samples_by_prompt, rewards2thresholds)
    )
    primary_reward_2_mrsd_pairs = balance_primary_axes(
        args, primary_reward_2_mrsd_pairs
    )
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=list,
        default=[
            "clap", "audiobox_aesthetics", "semantic_consistency"
        ],
    )
    parser.add_argument("--primary_axis_margin_perc", type=int, default=95)
    parser.add_argument(
        "--secondary_axis_margin_perc", type=int, default=50
    )
    parser.add_argument(
        "--num_samples_to_upload_per_reward", type=int, default=20
    )
